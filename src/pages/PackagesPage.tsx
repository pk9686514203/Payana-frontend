...
export default function PackagesPage() {
  ...
  const { data: packages, isLoading } = useApprovedPackages();

  const filtered = (packages || []).filter(
    (p) =>
      p.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
      p.locations?.some((l) => l.toLowerCase().includes(searchTerm.toLowerCase())) ||
      p.agents?.agency_name?.toLowerCase().includes(searchTerm.toLowerCase())
  );
        ...
        <div className="container mx-auto px-4 relative">
          ...
          <div className="mt-6 flex flex-col sm:flex-row gap-3 max-w-xl">
            <div className="relative flex-1">
              ...
              <input type="text" placeholder="Search packages, locations, or agencies..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
                ...
                <div className="bg-card rounded-2xl overflow-hidden shadow-card hover:shadow-card-hover transition-all duration-300 border border-border/50">
                  ...
                  <div className="p-5">
                    ...
                    <div className="flex items-center justify-between mt-4">
                      ...
                      <Button size="sm" className="bg-gradient-warm text-primary-foreground hover:opacity-90 rounded-lg text-xs shadow-sm">Book Now</Button>
                    </div>
                  </div>
                </div>
              </Link>
