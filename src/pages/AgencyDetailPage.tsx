...
import { ArrowLeft, MapPin, Star, CheckCircle, Package, Phone, Mail, Instagram } from "lucide-react";
import { Button } from "@/components/ui/button";
import Layout from "@/components/Layout";
import { mockAgencies, mockPackages } from "@/data/mockData";

export default function AgencyDetailPage() {
  const { id } = useParams();
  const agency = mockAgencies.find((a) => a.id === id);
  const agencyPackages = mockPackages.filter((p) => p.agencyId === id);
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="bg-card rounded-2xl p-6 md:p-8 shadow-card mb-8 border border-border/50">
          <div className="flex flex-col md:flex-row items-start gap-6">
            {agency.logo ? (
              <img src={agency.logo} alt={agency.name} className="w-20 h-20 rounded-2xl object-cover shadow-sm shrink-0" />
            ) : (
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-muted-foreground flex items-center gap-1 mb-3">
                <MapPin className="h-4 w-4" /> {agency.location} • Owned by {agency.owner}
              </p>
              <p className="text-muted-foreground">{agency.description}</p>
              <div className="flex items-center gap-4 mt-4">
                <span className="flex items-center gap-1"><Star className="h-4 w-4 fill-orange text-orange" /> {agency.rating} rating</span>
                <span className="flex items-center gap-1 text-sm text-muted-foreground"><Package className="h-4 w-4" /> {agencyPackages.length} packages</span>
              </div>
              {/* Contact details */}
        ...
        <h2 className="text-xl font-bold text-foreground mb-6">Packages by {agency.name}</h2>
            ...
            {agencyPackages.map((pkg) => (
              <Link key={pkg.id} to={`/packages/${pkg.id}`} className="group block">
                <div className="bg-card rounded-2xl overflow-hidden shadow-card hover:shadow-card-hover transition-all duration-300 border border-border/50">
                  ...
                  <div className="p-5">
                    <h3 className="font-bold text-foreground group-hover:text-secondary transition-colors">{pkg.name}</h3>
